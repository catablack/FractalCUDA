...
//
//__host__ __device__ void set_pixel(unsigned char* image, int width, int x, int y, unsigned char *c) {
...
void description(char* name, char* desc) {
	...
	else if (strcmp("gpu", name) == 0) {
		sprintf(desc, "width=%d height=%d iterations=%d grid size=%dx%d block size=%dx%d ", WIDTH, HEIGHT, MAX_ITERATION, GRID_SIZE_X, GRID_SIZE_Y, BLOCK_SIZE_X, BLOCK_SIZE_Y);
	}
    else {
        sprintf(desc, "-");
    }
